...
export default function Orders({ ordersData, setOrdersData, ordersLoaded, setOrdersLoaded, ordersStore, setOrdersStore, cfUrl, activePeriod, onPeriodChange }) {
  ...
  // Line item ka current SKU nikalta hai. Priority: line item ka apna stored sku
  // (order ke purchase-time ka frozen record — Shopify variant_id ko baad mein
  // kisi bilkul different variant ke liye REUSE kar sakta hai, is liye "live"
  // lookup order ke asal SKU se unrelated cheez wapas de sakta hai; raw sku hamesha
  // sahi hota hai jab tak khaali na ho) -> variant_id se fresh products_cache lookup
  // (sirf jab raw sku khaali ho) -> product_id + variant_title se lookup (variant
  // delete/recreate ke baad naya variant_id mila ho to bhi SKU recover ho jaye).
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 0, position: "sticky", top: 0, zIndex: 5, background: "var(--ne-surface-2)", border: "1px solid var(--ne-border)", borderRadius: 14, boxShadow: "0 2px 8px rgba(0,0,0,.18)", overflow: "hidden",
              ...
              <div ref={registerMiddleRef("header")} onScroll={handleMiddleScroll("header")} className="ne-hide-scroll"
                ...
                <div style={{ display: "flex", gap: 10, width: MIDDLE_CONTENT_WIDTH }}>
                  ...
                  <span style={{ ...thBase, background: "none", border: "none", padding: 0, width: 190, flexShrink: 0 }}>{t("orders.address")}</span>
                  <span style={{ ...thBase, background: "none", border: "none", padding: 0, width: 160, flexShrink: 0 }}>{t("orders.items")}</span>
