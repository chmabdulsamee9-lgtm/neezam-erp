...
export default function SupplierLedger({ storeId }) {
  ...
  const addSupplier = async (e) => {
    e.preventDefault();
    setSupplierError("");
    if (!supplierForm.name.trim()) { setSupplierError("Naam daalo"); return; }
  ...
  const addEntry = async (e) => {
    e.preventDefault();
    setEntryError("");
    if (!entryForm.amount || Number(entryForm.amount) <= 0) { setEntryError("Valid amount daalo"); return; }
    setEntrySaving(true);
      ...
      <div style={{ padding: isMobile ? "1rem" : "1.5rem", color: "var(--ne-text)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem", flexWrap: "wrap", gap: 10 }}>
          <div>
            ...
            <p style={{ margin: "2px 0 0", fontSize: 11.5, color: "var(--ne-muted)" }}>{viewingSupplier.contact_phone || "no phone"}</p>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ textAlign: "right" }}>
              ...
              <div style={{ fontSize: 10, color: "var(--ne-muted)" }}>{balance > 0 ? "Payable" : balance < 0 ? "Advance" : "Clear"}</div>
            </div>
        ...
        <div style={{ background: "var(--ne-surface-2)", border: "1px solid var(--ne-border)", borderRadius: 14, padding: "1rem" }}>
          {ledgerRows.length === 0 ? (
            <div style={{ textAlign: "center", padding: "2rem", color: "var(--ne-muted-2)", fontSize: 12 }}>Abhi koi entry nahi hai.</div>
          ) : isMobile ? (
                ...
                <div key={row.id} style={{ background: "var(--ne-surface)", border: "1px solid var(--ne-border)", borderRadius: 10, padding: "10px 12px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 10, fontWeight: 700, background: row.type === "debit" ? "var(--ne-danger-soft)" : "var(--ne-success-soft)", color: row.type === "debit" ? "var(--ne-danger)" : "var(--ne-success)" }}>
                      {row.type === "debit" ? "Maal Liya" : "Payment"}
                    </span>
                  </div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: 12.5, fontWeight: 700, color: "var(--ne-text)" }}>{rupees(row.amount)}</span>
                    <span style={{ fontSize: 11, color: "var(--ne-muted)" }}>Balance: {rupees(row.running)}</span>
                  </div>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5 }}>
                <thead>
                  <tr>
                    {["Date", "Type", "Amount", "Notes", "Running Balance"].map(h => (
          ...
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000000 }}>
            <div style={{ background: "var(--ne-surface-2)", border: "1px solid var(--ne-border)", borderRadius: 16, width: 380, maxWidth: "94vw", boxShadow: "0 12px 40px rgba(0,0,0,0.6)" }}>
              <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--ne-border)" }}>
                <h2 style={{ margin: 0, fontSize: 15, color: "var(--ne-text)" }}>+ Add Entry — {viewingSupplier.name}</h2>
              </div>
              <form onSubmit={addEntry} style={{ padding: "16px 18px" }}>
                ...
                <input type="number" placeholder="Amount (Rs.)" value={entryForm.amount} step="0.01"
                ...
                <textarea placeholder="Notes (optional)" rows={2} value={entryForm.notes}
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button type="submit" disabled={entrySaving}
                    style={{ flex: 1, padding: "10px", background: entrySaving ? "var(--ne-border)" : "var(--ne-success)", color: entrySaving ? "var(--ne-muted)" : "#0A2E1A", border: "none", borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: entrySaving ? "default" : "pointer" }}>
                    {entrySaving ? "Add ho raha hai..." : "✓ Add Entry"}
                  </button>
  ...
  return (
    <div style={{ padding: isMobile ? "1rem" : "1.5rem", color: "var(--ne-text)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem", flexWrap: "wrap", gap: 10 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>📒 Supplier Ledger</h1>
          <p style={{ margin: "2px 0 0", fontSize: 11.5, color: "var(--ne-muted)" }}>{suppliers.length} suppliers</p>
        </div>
            ...
            return (
              <div key={s.id} style={{ background: "var(--ne-surface-2)", border: "1px solid var(--ne-border)", borderRadius: 14, padding: "14px 16px", boxShadow: "0 2px 8px rgba(0,0,0,.18)", display: "flex", flexDirection: "column", gap: 10 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "var(--ne-muted-2)" }}>{s.contact_phone || "no phone"}</div>
                </div>
                <div style={{ background: "var(--ne-surface)", border: "1px solid var(--ne-border)", borderRadius: 10, padding: "8px 10px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <span style={{ fontSize: 11, color: "var(--ne-muted)" }}>{balance > 0 ? "Payable" : balance < 0 ? "Advance" : "Balance"}</span>
      ...
      {showAddSupplier && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000000 }}>
          <div style={{ background: "var(--ne-surface-2)", border: "1px solid var(--ne-border)", borderRadius: 16, width: 380, maxWidth: "94vw", boxShadow: "0 12px 40px rgba(0,0,0,0.6)" }}>
            <div style={{ padding: "14px 18px", borderBottom: "1px solid var(--ne-border)" }}>
              <h2 style={{ margin: 0, fontSize: 15, color: "var(--ne-text)" }}>+ Add Supplier</h2>
            </div>
            <form onSubmit={addSupplier} style={{ padding: "16px 18px" }}>
              <input type="text" placeholder="Naam" value={supplierForm.name}
                onChange={e => setSupplierForm(f => ({ ...f, name: e.target.value }))} style={inputStyle} />
              <input type="tel" placeholder="Phone number" value={supplierForm.contact_phone}
                onChange={e => setSupplierForm(f => ({ ...f, contact_phone: e.target.value }))} style={inputStyle} />
              <textarea placeholder="Notes (optional)" rows={2} value={supplierForm.notes}
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button type="submit" disabled={supplierSaving}
                  style={{ flex: 1, padding: "10px", background: supplierSaving ? "var(--ne-border)" : "var(--ne-success)", color: supplierSaving ? "var(--ne-muted)" : "#0A2E1A", border: "none", borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: supplierSaving ? "default" : "pointer" }}>
                  {supplierSaving ? "Add ho raha hai..." : "✓ Add Supplier"}
                </button>
